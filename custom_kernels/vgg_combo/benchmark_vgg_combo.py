"""
Benchmark — vgg_combo custom kernel vs ORT baseline.

Usage (from repo root):
    .venv\\Scripts\\python custom_kernels\\vgg_combo\\benchmark_vgg_combo.py

Optional env vars:
    ONNX_PATH=model_assets/vgg_combo_relu3_3_relu3_1.onnx
    WARMUP=20
    ITERS=200
"""
...
ONNX_PATH = pathlib.Path(
    os.environ.get(
        "ONNX_PATH",
        ROOT / "model_assets" / "vgg_combo_relu3_3_relu3_1.onnx",
    )
)
...
def bench():
    print("\n=== vgg_combo_relu3_3_relu3_1 — (1,3,512,512) → (1,512,128,128) ===")
    ...
    if "TensorrtExecutionProvider" not in _ort.get_available_providers():
        ...
        ctx = ROOT / "tensorrt-engines" / "vgg_combo_relu3_3_relu3_1_ctx.onnx"
        if not ctx.exists():
            print(
    ...
    m_fp32 = (
        VggComboTorch.from_onnx(ONNX_PATH, compute_dtype=torch.float32).cuda().eval()
    )
    with torch.no_grad():
    ...
    m_fp16 = (
        VggComboTorch.from_onnx(ONNX_PATH, compute_dtype=torch.float16).cuda().eval()
    )
    with torch.no_grad():
    ...
    from custom_kernels.vgg_combo.vgg_combo_torch import build_cuda_graph_runner

    runner = build_cuda_graph_runner(m_fp16)
    t3 = _bench(lambda: runner(inp))
    _print_row("3", "PyTorch FP16 + CUDA graph", t3, t0)

    # ── Tier 4: 2× batched (both faces in one pass) ───────────────────────
    inp2 = torch.randn(2, 3, 512, 512, dtype=torch.float32, device="cuda")
    runner2 = build_cuda_graph_runner(m_fp16, input_shape=(2, 3, 512, 512))
    ...
    mean_err = float(np.abs(ref_out - pt_out).mean())
    rel_err = max_err / feat_range * 100.0
    print(f"  Feature range : [{ref_out.min():.2f}, {ref_out.max():.2f}]")
    print(f"  max  |Δ| = {max_err:.4f}  ({rel_err:.3f}% of feature range)")
    print(f"  mean |Δ| = {mean_err:.4f}")
    if rel_err < 1.0:
        print("  ✓ Accuracy OK (relative error < 1%)")
    else:
