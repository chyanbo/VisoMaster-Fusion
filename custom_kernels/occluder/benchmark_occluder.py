"""
Benchmark — occluder.onnx (ResNet-encoder U-Net face occluder) custom kernels vs ORT.

Tests the full occluder pipeline:
  Input : (1, 3, 256, 256) float32  — RGB face crop, values in [0, 1]
  Output: (1, 1, 256, 256) float32  — raw logits (positive = face)
...
OCCLUDER_ONNX = str(ONNX_DIR / "occluder.onnx")
...
def _check_accuracy(ort_out: np.ndarray, pt_out: torch.Tensor) -> None:
    ...
    print(f"    Binary pixel agreement (threshold 0): {agree:.2f}%  (target > 99%)")
...
# ---------------------------------------------------------------------------
def bench_occluder():
    print("\n=== occluder / ResNet-encoder U-Net  (1,3,256,256) → (1,1,256,256) ===")
    ...
    sess0 = _ort_session(OCCLUDER_ONNX, "CUDAExecutionProvider")
    in_name = sess0.get_inputs()[0].name  # "img"
    ...
    if "TensorrtExecutionProvider" not in _ort.get_available_providers():
        ...
        ctx = ROOT / "tensorrt-engines" / "occluder_ctx.onnx"
        if not ctx.exists():
            print(
    ...
    m_fp32 = (
        OccluderTorch.from_onnx(OCCLUDER_ONNX, compute_dtype=torch.float32)
        .cuda()
        .eval()
    )
    with torch.no_grad():
    ...
    # ── Tier 2 — PyTorch FP16 ─────────────────────────────────────────────
    m_fp16 = (
        OccluderTorch.from_onnx(OCCLUDER_ONNX, compute_dtype=torch.float16)
        .cuda()
        .eval()
    )
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    if not _pl.Path(OCCLUDER_ONNX).exists():
        print(f"ERROR: occluder ONNX not found: {OCCLUDER_ONNX}")
        sys.exit(1)
