"""
Benchmark — fan_2dfan4 custom kernel vs ORT baseline.

Usage (from repo root):
    .venv\\Scripts\\python custom_kernels\\fan_2dfan4\\benchmark_fan_2dfan4.py

Optional env vars:
    ONNX_PATH=model_assets/2dfan4.onnx   path to ONNX model
...
ONNX_PATH = pathlib.Path(
    os.environ.get("ONNX_PATH", ROOT / "model_assets" / "2dfan4.onnx")
)
...
def bench():
    print("\n=== fan_2dfan4 face landmark detector — (1,3,256,256) → (1,68,3) ===")
    ...
    if "TensorrtExecutionProvider" not in _ort.get_available_providers():
        ...
        ctx = ROOT / "tensorrt-engines" / "2dfan4_ctx.onnx"
        if not ctx.exists():
            print(
    ...
    m_fp32 = FAN2dfan4.from_onnx(ONNX_PATH, compute_dtype=torch.float32).cuda().eval()
    with torch.no_grad():
        t1 = _bench(lambda: m_fp32(inp))
    _print_row("1", "PyTorch FP32 eager", t1, t0)

    # ── Tier 2: PyTorch FP16 eager ────────────────────────────────────────
    m_fp16 = FAN2dfan4.from_onnx(ONNX_PATH, compute_dtype=torch.float16).cuda().eval()
    with torch.no_grad():
        t2 = _bench(lambda: m_fp16(inp))
    _print_row("2", "PyTorch FP16 eager", t2, t0)

    # ── Tier 3: PyTorch FP16 + CUDA graph ─────────────────────────────────
    from custom_kernels.fan_2dfan4.fan_2dfan4_torch import build_cuda_graph_runner
    ...
    # ORT output[0] = landmarks_xyscore (1,68,3), output[1] = heatmaps (1,68,64,64)
    ...
    print(f"  max  |Δ| = {max_err:.4f}  (landmarks_xyscore x,y in [0.5..63.5])")
    print(f"  mean |Δ| = {mean_err:.6f}")
    if max_err < 1.0:
        print("  ✓ Accuracy OK (max error < 1.0 pixel)")
    else:
