"""
Benchmark — face_landmark478 custom kernel vs ORT baseline.

Usage (from repo root):
    .venv\\Scripts\\python custom_kernels\\face_landmark478\\benchmark_face_landmark478.py

Optional env vars:
    ONNX_PATH=model_assets/face_landmarks_detector_Nx3x256x256.onnx
    WARMUP=50
    ITERS=500
"""
...
ONNX_PATH = pathlib.Path(
    os.environ.get(
        "ONNX_PATH",
        ROOT / "model_assets" / "face_landmarks_detector_Nx3x256x256.onnx",
    )
)
...
def bench():
    print("\n=== face_landmark478 detector — (1,3,256,256) → (1,1,1,1434) ===")
    ...
    if "TensorrtExecutionProvider" not in _ort.get_available_providers():
        ...
        ctx = ROOT / "tensorrt-engines" / "face_landmarks_detector_Nx3x256x256_ctx.onnx"
        if not ctx.exists():
            print(
    ...
    m_fp32 = (
        FaceLandmark478Torch.from_onnx(ONNX_PATH, compute_dtype=torch.float32)
        .cuda()
        .eval()
    )
    with torch.no_grad():
    ...
    m_fp16 = (
        FaceLandmark478Torch.from_onnx(ONNX_PATH, compute_dtype=torch.float16)
        .cuda()
        .eval()
    )
    with torch.no_grad():
    ...
    # ref_outs[0] = Identity (1,1,1,1434) = landmarks
    ref_lmk = ref_outs[0].reshape(478, 3)  # (478, 3) ORT
    with torch.no_grad():
        pt_outs = runner(inp)
    pt_lmk = pt_outs[0].cpu().numpy().reshape(478, 3)  # (478, 3) PyTorch

    max_err = float(np.abs(ref_lmk - pt_lmk).max())
    mean_err = float(np.abs(ref_lmk - pt_lmk).mean())
    print(f"  max  |Δ| = {max_err:.4f}  (landmarks x/y/z)")
    print(f"  mean |Δ| = {mean_err:.6f}")
    if max_err < 1.0:
        print("  ✓ Accuracy OK (max error < 1.0)")
    else:
