...
def handle_auto_mouth_toggle(main_window: "MainWindow", new_value: bool) -> None:
    ...
    When enabled, ensures the landmark detector is on and set to a compatible
    model ('203' or '68').  Switches silently to '203' if the current model is
    incompatible.
    """
    if not new_value:
        return

    # Ensure landmark detection is enabled
    if not main_window.control.get("LandmarkDetectToggle", False):
        main_window.control["LandmarkDetectToggle"] = True
        handle_landmark_state_change(main_window, True, "LandmarkDetectToggle")
    ...
    # Ensure a compatible landmark model is selected
    ...
    if current_model not in ("203", "68"):
        ...
        handle_landmark_model_selection_change(
            main_window, "203", "LandmarkDetectModelSelection"
        )
...
def set_auto_mouth_threshold_from_frame(main_window: "MainWindow", *_args) -> None:
    """Measure the lip-open ratio on the current frame and set it as the threshold.

    Face selection heuristic: face whose 5-pt kps centre is closest to the
    source face centre of the selected target button; fallback to largest bbox.
    """
    import traceback
    import numpy as np
    ...
    target_face = main_window.cur_selected_target_face_button
    if target_face is None:
        print("[WARN] set_auto_mouth_threshold_from_frame: No target face selected.")
        return

    raw_frame = getattr(main_window.video_processor, "current_frame", None)
    if raw_frame is None or (hasattr(raw_frame, "__len__") and len(raw_frame) == 0):
        print("[WARN] set_auto_mouth_threshold_from_frame: No current frame available.")
        return
    ...
    try:
        ...
        # Determine landmark model (force to "203" if incompatible)
        lmk_model = control.get("LandmarkDetectModelSelection", "203")
        if lmk_model not in ("203", "68"):
            lmk_model = "203"

        # Convert frame to CHW tensor (frame is already RGB numpy HWC)
        if isinstance(raw_frame, np.ndarray):
            frame_chw = torch.from_numpy(np.ascontiguousarray(raw_frame)).permute(
                2, 0, 1
            )
        else:
            frame_chw = raw_frame  # already tensor

        detector_model = control.get("DetectorModelSelection", "RetinaFace")
        ...
        if not bboxes or len(bboxes) == 0:
            print(
                "[WARN] set_auto_mouth_threshold_from_frame: No faces detected in frame."
            )
            return

        # Determine source kps centre for face selection heuristic
        source_centre: "np.ndarray | None" = None
        if target_face.assigned_input_faces:
            first_id = list(target_face.assigned_input_faces.keys())[0]
            store = target_face.assigned_input_faces[first_id]
            if "kps_5" in store:
                source_centre = np.mean(store["kps_5"], axis=0)

        best_idx = 0
        if source_centre is not None and len(kpss_5) > 0:
            # Closest kps centre to source
            min_dist = float("inf")
            for i, kps in enumerate(kpss_5):
                centre = np.mean(kps, axis=0)
                dist = float(np.linalg.norm(centre - source_centre))
                if dist < min_dist:
                    min_dist = dist
                    best_idx = i
        else:
        ...
        best_kps_5 = kpss_5[best_idx] if len(kpss_5) > best_idx else None

        # Run landmark detection
        kpss_5_lm, kpss_all_lm, _ = models_processor.run_detect_landmark(
            img=frame_chw,
            bbox=best_bbox,
            det_kpss=best_kps_5,
            detect_mode=lmk_model,
            score=float(control.get("LandmarkDetectScoreSlider", 50)) / 100.0,
        )

        if not kpss_all_lm or len(kpss_all_lm) == 0:
            print(
                "[WARN] set_auto_mouth_threshold_from_frame: "
                "Landmark detection returned no keypoints."
            )
            return

        kps_all = kpss_all_lm[0]
        ...
        if lmk_model == "203":
            ratio = compute_lip_open_ratio_203(kps_all)
        elif lmk_model == "68":
            ratio = compute_lip_open_ratio_68(kps_all)

        if ratio is None:
            print(
                "[WARN] set_auto_mouth_threshold_from_frame: "
                "Could not compute lip-open ratio (face too small or not enough landmarks)."
            )
            return
        ...
        if threshold_widget is not None:
            from app.ui.widgets.widget_components import ParameterDecimalSlider

            if isinstance(threshold_widget, ParameterDecimalSlider):
                threshold_widget.set_value(clamped)

        print(
            f"[INFO] set_auto_mouth_threshold_from_frame: "
            f"ratio={ratio:.4f} → threshold set to {clamped:.4f}"
        )

    except Exception as exc:
        print(f"[ERROR] set_auto_mouth_threshold_from_frame: {exc}")
        traceback.print_exc()
